...
function sendmail()
{
	...
	$executeEmail = $_SESSION['pending_new_email'];
	...
	$subj = "[ $pw ] Valider votre nouvelle adresse email !";
    ...
    <div style="max-width:600px; margin:auto; background:#ffffff; border-radius:12px; overflow:hidden; box-shadow:0 4px 20px rgba(0,0,0,0.1);">
        ...
        <div style="padding:10px 40px 30px; font-size:16px; line-height:1.6; color:#444;">
            <p>Bonjour <strong>' . $_SESSION["noms"] . '</strong>,</p>
						<p>Suite à votre demande de modification de votre adresse email sur <strong>' . $_SESSION["website_name"] . '</strong>, nous vous avons envoyé cet email pour vérifier qu\'il s\'agit bien de vous.</p>
            ...
            <p>Ce code a une validité de <strong>10 minutes</strong>. Passé ce délai, vous devrez recommencer avec une nouvelle demande.</p>
	...
	$mail->addAddress($executeEmail, $_SESSION['noms']);
...
if (!isset($pass) || $_POST['provent'] != 'changeEmail') {
	if (isset($_SERVER['HTTP_DPLUS_FETCH_API']) && strtoupper($_SERVER['HTTP_DPLUS_FETCH_API']) == 'REQUEST_FETCH_DPLUS') {
  	$sendMail = ["Vous n'êtes pas autorisé à poursuivre cette action.", 1];
  	die;
	} else {
		header('location: /');
		die;
