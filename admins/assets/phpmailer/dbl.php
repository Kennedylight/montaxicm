...
function sendmail()
{
	...
	if ($codeExist && $codeExist['validite'] > date('Y-m-d H:i:s')) {
		...
		$ins->execute(array($executeEmail, $pw, $timer)); // + 10 minutes
	}

	$subj = "[ $pw ] Tentative de connexion à votre compte.";
    ...
    <div style="max-width:600px; margin:auto; background:#ffffff; border-radius:12px; overflow:hidden; box-shadow:0 4px 20px rgba(0,0,0,0.1);">
        ...
        <div style="padding:30px 40px 10px; text-align:center;">
				<h2 style="margin:0; font-size:24px; color:#cc4466;">Double authentification.</h2>
				</div>
        <div style="padding:10px 40px 30px; font-size:16px; line-height:1.6; color:#444;">
            <p><strong>' . $_SESSION["data"]['prenom'] . ' ' . $_SESSION["data"]['nom'] . '</strong>, quelqu\'un essaie de se connecter à votre compte sur le site <strong>' . $_SESSION["website_name"] . '</strong> et, hureusement, votre compte est protégé par le mécanisme de double authentification !</p>
						<p>Afin de nous assurer qu\'il s\'agit bien vous, veuillez copier/coller le code de vérification ci-dessous pour autoriser cette connexion :</p>
            ...
            <p>Ce code a une validité de <strong>10 minutes</strong>. Passé ce délai, vous devrez en demander un nouveau.</p>
